  ...
  <div class="rk-trace-search">
    <div>
      ...
      <a class="rk-trace-clear-btn r mr-10" @click="clearSearch">
       ...
       <a class="rk-trace-search-btn bg-blue r mr-10" @click="getTraceListByTime">
        <svg class="icon mr-5 vm">
          <use xlink:href="#export"></use>
        </svg>
        <span class="vm">{{ this.$t("exportTrace") }}</span>
      </a>
      <div class="flex-h">
        <TraceSelect
          :hasSearch="true"
...
export default class TraceSearch extends Vue {
  ...
  @Action("rocketTrace/GET_TRACELISTBYTIME") private GET_TRACELISTBYTIME: any;
  @Action("rocketTrace/SET_TRACE_FORM") private SET_TRACE_FORM: any;
  @Action("rocketTrace/SET_TRACEBYTIME_FORM") private SET_TRACEBYTIME_FORM: any;
  ...
  private inTopo!: boolean;
   @Watch('rocketTrace.traceListByTime')
  private onTraceListChange() {
    if (this.rocketTrace.traceListByTime && this.rocketTrace.traceListByTime.length > 0) {
        let blob = new Blob([JSON.stringify(this.rocketTrace.traceListByTime)], { type: 'application/json' })
  ...
  private getTraceListByTime() {
    const temp: any = 
       this.globalTimeFormat([
        new Date(
          this.time[0].getTime() +
            (parseInt(this.rocketbotGlobal.utc, 10) +
              new Date().getTimezoneOffset() / 60) *
    ...
    this.SET_TRACEBYTIME_FORM(temp);
    this.$eventBus.$emit("SET_LOADING_TRUE", () => {
      this.GET_TRACELISTBYTIME().then(() => {
        this.$eventBus.$emit("SET_LOADING_FALSE");
